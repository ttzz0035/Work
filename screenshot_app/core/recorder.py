...
class InputRecorder(QtCore.QObject):
    """RegionWindow からの入力を記録（停止時に NDJSON へ書き出し）"""
    ...
    def start(self, out_path: Path, base_win: QtCore.QRect):
        ...
        log.info("[REC] start -> %s", self._out_path)

    def stop(self) -> Optional[Path]:
        self._active = False
        if not self._out_path: return None
        p = self._out_path
        with p.open("w", encoding="utf-8") as f:
            for ev in self._events:
                f.write(json.dumps(asdict(ev), ensure_ascii=False) + "\n")
        log.info("[REC] saved %d events -> %s", len(self._events), p)
        return p

    def _now(self) -> float:
        return 0.0 if self._base_t is None else (time.perf_counter() - self._base_t)

    # --- hooks (RegionWindow から呼ぶ) ---
...
class InputPlayer(QtCore.QObject):
    """NDJSON を読み込み、対象ウィジェットに postEvent で再生"""
    ...
    def __init__(self, target_widget: QtWidgets.QWidget, parent=None):
        ...
        self._events: List[RecEvent] = []
        self._timer = QtCore.QTimer(self); self._timer.setSingleShot(True); self._timer.timeout.connect(self._step)
        self._i = 0
        self._t0 = 0.0
    ...
    def load(self, path: Path):
        self._events.clear(); self._base_win = None
    ...
    def _schedule_next(self):
        if self._stopped or self._i >= len(self._events):
            self.finished.emit(); return
    ...
    def _dispatch(self, ev: RecEvent):
        if ev.kind == "mouse": self._dispatch_mouse(ev)
        elif ev.kind == "key": self._dispatch_key(ev)
    ...
    def _dispatch_mouse(self, ev: RecEvent):
        ...
        if ev.etype == "move":
            ...
            we = QtGui.QWheelEvent(pos, gpos, QtCore.QPoint(0, ev.delta or 0),
                                   ...
                                   QtCore.Qt.MouseButtons(ev.buttons or 0),
                                   QtCore.Qt.KeyboardModifiers(0))
            QtGui.QGuiApplication.postEvent(self._target, we); return
        else:
            return
        QtGui.QGuiApplication.postEvent(self._target, me)

    def _dispatch_key(self, ev: RecEvent):
        kp = QtGui.QKeyEvent(QtCore.QEvent.KeyPress, ev.key or 0,
                             QtCore.Qt.KeyboardModifiers(ev.mods or 0), ev.text or "")
        kr = QtGui.QKeyEvent(QtCore.QEvent.KeyRelease, ev.key or 0,
                             QtCore.Qt.KeyboardModifiers(ev.mods or 0), ev.text or "")
